...
if ($checkId->code == 0) {
  echo "<script>alert('팝빌 아이디를 확인해 주세요.');</script>";
...
if ($corpinfo->corpName == null || $corpinfo->corpName == '') {
    echo "<script>alert('사업자번호와 팝빌 아이디를 확인해 주세요.');</script>";
//   echo "<script>history.back();</script>";
  exit();
}
...
if (!$gong) {
  ...
  if (strpos($gong, '-99910004')) {
    echo "<script>alert('팝빌사이트에 공인인증서를 등록해야 전자세금계산서 발행이 가능합니다.');</script>";
    //   echo "<script>history.back();</script>";
    exit();
  }
}
...
for ($i = 0; $i < count($a); $i++) {
  ...
  $Taxinvoice->detailList[0]->itemName = '이용료';  // 품명
